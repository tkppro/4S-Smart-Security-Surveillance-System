...
def main_loop():
    # video_capture = cv2.VideoCapture('http://admin:admin@192.168.0.105/videostream.cgi?user=admin&pwd=admin&resolution=32&rate=0')
    ...
    while True:
        ...
        for face_location, face_encoding in zip(face_locations, face_encodings):
            ...
            if metadata is not None:
                ...
                name = metadata['name']
                face_label = f"{name} - {int(time_at_door.total_seconds())}s"
                
                # face_labels.append(name)
                if time_at_door > timedelta(seconds=30) and metadata['send_data'] is False:
                    print('Update visible time on firebase.....')
                    metadata['send_data'] = True
